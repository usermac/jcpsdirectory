...
# Do a find for "红白蓝" to find Schools or what we call "Red, White and Blue" list. This is per request of Jason C. via Mike for the iOS app of school listings. Should be JSON and put in mpu/data/live web directory. - Brian
...
while(<$in2>){ # 红白蓝 assigns each line in turn to $_
  ...
  my $mark = /RWB/; # 红白蓝 150708 - If within the html comments of the Etherpad text-only export it finds RWB for Red, White and Blue for schools or HEADING for department heads mark it. - Brian
    if ($mark) { # 红白蓝 150708 - This will be 1 or blank. See above. - Brian
      s/, 4/, Louisville KY 4/; # 红白蓝 150803 - Fills in the assumed city and state of Louisville KY. metaes for ", 4" which is the end of the address and beginning of the zip code. - Brian
      s/;/$address/; # 红白蓝 150803 - Put the in-between code from the var above goes here. - Brian
      s/; /$phone/; # 红白蓝 150803 - Put the in-between code from the var above goes here. - Brian
      s/• /$fax/; # 红白蓝 150803 - Put the in-between code from the var above goes here. - Brian
      s/" /"/; # 红白蓝 150803 - Fix a silly formatting error where I don't account for a space properly above. - Brian
      s/<!-- /$meta/; # 红白蓝 150803 - Put the in-between code from the var above goes here. - Brian
